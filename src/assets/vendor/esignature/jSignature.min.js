...
(function () {
  function t(b) {
    ...
    e
      ...
      : (d = m
          ? 127 < Math.max.apply(null, [m.r, m.g, m.b])
            ? { r: 0, g: 0, b: 0 }
            : { r: 255, g: 255, b: 255 }
          : { r: 255, g: 255, b: 255 });
    a = function (a) {
    ...
    m && d
      ? ((b = Math.max.apply(null, [m.r, m.g, m.b])),
        (m = Math.max.apply(null, [d.r, d.g, d.b])),
        (m = Math.round(m + -0.75 * (m - b))),
        (m = { r: m, g: m, b: m }))
      : m
        ? ((m = Math.max.apply(null, [m.r, m.g, m.b])),
          (b = 1),
          127 < m && (b = -1),
          (m = Math.round(m + 96 * b)),
          (m = { r: m, g: m, b: m }))
        : (m = { r: 191, g: 191, b: 191 });
    return { color: c, 'background-color': d ? a(d) : e, 'decor-color': a(m) };
  }
  function k(b, a) {
    this.x = b;
    this.y = a;
    ...
    this.angleTo = function (a) {
      ...
      return 0 === b
        ? 0
        : Math.acos(
            Math.min(Math.max((this.x * a.x + this.y * a.y) / b, -1), 1)
          ) / Math.PI;
    };
  }
  function g(b, a) {
  ...
  function n(b, a, c, e, h) {
    ...
    this.addToStroke = function (a) {
      if (
        ...
        'number' === typeof a.y &&
        4 <
          Math.abs(a.x - this._lastPoint.x) + Math.abs(a.y - this._lastPoint.y)
      ) {
  ...
  function r(b, a, c, e) {
    if ('ratio' === a || '%' === a.split('')[a.length - 1])
      this.eventTokens[c + '.parentresized'] = e.subscribe(
        c + '.parentresized',
        (function (a, m, d, l) {
          return function () {
            var h = m.width();
            if (h !== d) {
              for (var l in a)
                a.hasOwnProperty(l) && (e.unsubscribe(a[l]), delete a[l]);
              var f = b.settings;
              ...
              for (g = l.length; E < g; E++) {
                ...
                A = { x: [], y: [] };
                k = 0;
                for (r = n.x.length; k < r; k++)
                  A.x.push(n.x[k] * h), A.y.push(n.y[k] * h);
                u.push(A);
              }
              f.data = u;
              m[c](f);
  ...
  function x(b, a, c) {
    ...
    e = function (a) {
        ...
        e = function (e) {
          e =
            e.changedTouches && 0 < e.changedTouches.length
              ...
              : e;
          return new g(
            Math.round(e.pageX + b),
            Math.round(e.pageY + c) + a.fatFingerCompensation
          );
        },
      ...
      this.drawMoveHandler = function (b) {
        a.settings.readOnly ||
          (b.preventDefault(),
          a.dataEngine.inStroke && (a.dataEngine.addToStroke(e(b)), d.kick()));
      };
      return this;
    }.call({}, this);
    (function (a, b, c) {
      ...
      this.isCanvasEmulator
        ? (h.bind('mousemove.jSignature', c),
          h.bind('mouseup.jSignature', a),
          h.bind('mousedown.jSignature', b))
    ...
    b['jSignature.windowmouseup'] = h.subscribe(
  ...
  function y(b) {
    ...
    if (c) {
      ...
      a &&
        a.screen &&
        a.screen.deviceXDPI &&
        a.screen.logicalXDPI &&
        (c = (1 * a.screen.deviceXDPI) / a.screen.logicalXDPI);
      if (1 !== c)
        try {
          $(b)
            .children('object')
            .get(0)
            .resize(Math.ceil(b.width * c), Math.ceil(b.height * c)),
  ...
  var w = function (b, a) {
    ...
    v = function (b) {
      ...
      this.subscribe = function (a, b, e) {
        this.topics[a]
          ? this.topics[a].push([b, e])
          : (this.topics[a] = [[b, e]]);
        return { topic: a, callback: b };
      };
      this.unsubscribe = function (a) {
        if (this.topics[a.topic])
          for (var b = this.topics[a.topic], e = 0, d = b.length; e < d; e++)
            b[e] && b[e][0] === a.callback && b.splice(e, 1);
      };
    },
    ...
    f = function (b, a) {
      ...
      if (1 < a) {
        ...
        if (f.getLength() > this.lineCurveThreshold) {
          l =
            2 < a
              ? new g(b.x[a - 3], b.y[a - 3]).getVectorToPoint(m)
              : new k(0, 0);
          var n = 0.35 * f.getLength(),
            p = f.angleTo(l.reverse()),
            q = d.angleTo(f.reverse());
          l = new k(l.x + f.x, l.y + f.y).resizeTo(Math.max(0.05, p) * n);
          var u = new k(f.x + d.x, f.y + d.y)
              .reverse()
              .resizeTo(Math.max(0.05, q) * n),
      ...
      d.getLength() <= this.lineCurveThreshold &&
        z(this.canvasContext, c.x, c.y, e.x, e.y);
    },
    d = function (b) {
      var a = b.x.length - 1;
      if (0 < a) {
  ...
  x.prototype.resetCanvas = function (b, a) {
    ...
    if (e.signatureLine) {
      ...
      m ||
        ((h.shadowColor = h.strokeStyle),
        (h.shadowOffsetX = 0.5 * h.lineWidth),
        (h.shadowOffsetY = -0.6 * h.lineWidth),
        (h.shadowBlur = 0));
    }
  ...
  (function (b) {
    ...
    function c(a, b) {
      this.find('canvas.jSignature')
        .add(this.filter('canvas.jSignature'))
    ...
    function d(a, b) {
      if (
        ...
        'string' === typeof a &&
        'data:' === a.substr(0, 5) &&
        ((b = a.slice(5).split(',')[0]), (a = a.slice(6 + b.length)), b === a)
      )
        return;
      var c = this.find('canvas.jSignature').add(
        this.filter('canvas.jSignature')
      );
      if (l.hasOwnProperty(b))
        ...
        throw Error(
          "jSignature is unable to find import plugin with for format '" +
            String(b) +
            "'"
        );
      return this;
    ...
    var f = {},
      ...
      p = { export: g, import: l, instance: f },
      n = {
        init: function (a) {
        ...
        getSettings: function () {
          return this.find('canvas.jSignature')
            .add(this.filter('canvas.jSignature'))
        ...
        isModified: function () {
          return (
            ...
            this.find('canvas.jSignature')
              .add(this.filter('canvas.jSignature'))
              .data('jSignature.this').dataEngine._stroke
          );
        },
        updateSetting: function (a, b, c) {
          var d = this.find('canvas.jSignature')
            .add(this.filter('canvas.jSignature'))
            .data('jSignature.this');
          d.settings[a] = b;
        ...
        getData: function (a) {
          var b = this.find('canvas.jSignature').add(
            this.filter('canvas.jSignature')
          );
          void 0 === a && (a = 'default');
          if (0 !== b.length && g.hasOwnProperty(a))
            return g[a].call(
              b.get(0),
              b.data('jSignature.data'),
              b.data('jSignature.settings')
            );
        },
        importData: d,
        setData: d,
        ...
        disable: function () {
          ...
          this.find('canvas.jSignature')
            .addClass('disabled')
            .data('jSignature.this').settings.readOnly = !0;
        },
        enable: function () {
          this.find('input').removeAttr('disabled');
          this.find('canvas.jSignature')
            .removeClass('disabled')
            .data('jSignature.this').settings.readOnly = !1;
        },
        events: function () {
          return this.find('canvas.jSignature')
            .add(this.filter('canvas.jSignature'))
...
(function () {
  for (
    var t = {},
      k = {},
      g = '0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWX'.split(
        ''
      ),
      n = g.length / 2,
      r = n - 1;
  ...
  var x = function (d) {
    ...
    y = function (d) {
      for (var b = [], a = 0, c = 1, e = d.length, h, f, g = 0; g < e; g++)
        (h = Math.round(d[g])),
          (f = h - a),
          (a = h),
          0 > f && 0 < c
            ? ((c = -1), b.push('Z'))
            : 0 < f && 0 > c && ((c = 1), b.push('Y')),
          (h = Math.abs(f)),
          h >= n ? b.push(x(h.toString(n))) : b.push(h.toString(n));
      return b.join('');
    },
    w = function (d) {
      var b = [];
      d = d.split('');
      for (var a = d.length, c, e = 1, f = [], m = 0, g = 0; g < a; g++)
        (c = d[g]),
          c in t || 'Z' === c || 'Y' === c
            ? (0 !== f.length &&
                ((f = parseInt(f.join(''), n) * e + m), b.push(f), (m = f)),
              'Z' === c
                ? ((e = -1), (f = []))
                : 'Y' === c
                  ? ((e = 1), (f = []))
    ...
    v = function (d) {
      for (var b = [], a = d.length, c, e = 0; e < a; e++)
        (c = d[e]), b.push(y(c.x)), b.push(y(c.y));
      return b.join('_');
    },
    z = function (d) {
      var b = [];
      d = d.split('_');
      for (var a = d.length / 2, c = 0; c < a; c++)
        b.push({ x: w(d[2 * c]), y: w(d[2 * c + 1]) });
      return b;
    },
    D = function (d) {
      return ['image/jsignature;base30', v(d)];
    },
    f = function (d, b, a) {
      'string' === typeof d &&
        ('image/jsignature;base30' === d.substring(0, 23).toLowerCase() &&
          (d = d.substring(24)),
        a(z(d)));
    };
  if (null == this.jQuery)
...
(function () {
  ...
  function k(f, d) {
    ...
    this.getVectorToPoint = function (b) {
      return new t(b.x - this.x, b.y - this.y);
    };
  ...
  function n(f, d, b) {
    ...
    return a.getLength() > b
      ? ((b =
          2 < d
            ? new k(f.x[d - 3], f.y[d - 3]).getVectorToPoint(e)
            : new t(0, 0)),
        (f = 0.35 * a.getLength()),
        (e = a.angleTo(b.reverse())),
        (d = c.angleTo(a.reverse())),
        (b = new t(b.x + a.x, b.y + a.y).resizeTo(Math.max(0.05, e) * f)),
        (c = new t(a.x + c.x, a.y + c.y)
          .reverse()
          .resizeTo(Math.max(0.05, d) * f)),
        (c = new t(a.x + c.x, a.y + c.y)),
        ['c', g(b.x, 2), g(b.y, 2), g(c.x, 2), g(c.y, 2), g(a.x, 2), g(a.y, 2)])
      : ['l', g(a.x, 2), g(a.y, 2)];
  }
  function r(f, d) {
    var b = f.x.length - 1,
    ...
    if (1 < b && a.getLength() > d) {
      var b = new k(f.x[b - 2], f.y[b - 2]).getVectorToPoint(c),
        ...
        e = 0.35 * a.getLength(),
        b = new t(b.x + a.x, b.y + a.y).resizeTo(Math.max(0.05, c) * e);
      return [
        'c',
        g(b.x, 2),
        g(b.y, 2),
  ...
  function x(f, d, b) {
    ...
    0 < a
      ? d.push.apply(d, r(f, b, 1))
      : 0 === a && d.push.apply(d, ['l', 1, 1]);
    return d.join(' ');
  }
  function y(f) {
    for (
  ...
  function w(f, d) {
    ...
    if (0 !== c) {
      for (a = 0; a < c; a++) {
        ...
        var p, q;
        p = 0;
        for (q = k.x.length; p < q; p++) l.push({ x: k.x[p], y: k.y[p] });
        l = simplify(l, 0.7, !0);
        p = 0;
    ...
    for (c = n.length; a < c; a++)
      (e = n[a]), b.push('<path ' + y(d) + ' d="' + x(e, k, l) + '"/>');
    b.push('</svg>');
    return b.join('');
  }
  function v(f, d) {
      ...
      for (e[g] = e[k] = 1; k; ) {
        ...
        for (l = g + 1; l < k; l++) {
          ...
          var B = f[g],
            ...
            y;
          if (0 !== B || 0 !== C)
            (y = ((p.x - v) * B + (p.y - w) * C) / (B * B + C * C)),
              1 < y
                ? ((v = x.x), (w = x.y))
                : 0 < y && ((v += B * y), (w += C * y));
          p = ((B = p.x - v), (C = p.y - w), B * B + C * C);
          p > n && ((q = l), (n = p));
        }
