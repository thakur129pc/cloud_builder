...
(function ($) {
    ...
    var tpl =
      ...
      fillHSBFields = function (hsba, cal) {
        $(cal)
          .data('wcolpick')
          .fields.eq(4)
          .val(Math.round(hsba.h))
          .end()
          .eq(5)
          .val(Math.round(hsba.s))
          .end()
          .eq(6)
          .val(Math.round(hsba.b))
          .end();
      },
      fillAlphaField = function (hsba, cal) {
        $(cal)
          .data('wcolpick')
          .fields.eq(7)
          .val(Math.round(hsba.a * 100))
          .end();
      },
      fillHexField = function (hex, cal) {
        if ($(cal).data('wcolpick').enableAlpha)
          $(cal).data('wcolpick').fields.eq(0).val(hex);
      ...
      setSelectorPos = function (hsba, cal) {
        $(cal)
          ...
          .selectorIndic.css({
            ...
            top: Math.round(
              ($(cal).data('wcolpick').size * (100 - hsba.b)) / 100
            ),
          });
      },
      ...
      setHuePos = function (hsba, cal) {
        $(cal)
          ...
          .hue.css(
            'top',
            Math.round(
              $(cal).data('wcolpick').size -
                ($(cal).data('wcolpick').size * hsba.h) / 360
            )
          );
      },
      ...
      setAlphaPos = function (hsba, cal) {
        if ($(cal).data('wcolpick').enableAlpha)
          $(cal)
            ...
            .alpha.css(
              'left',
              Math.round($(cal).data('wcolpick').size * hsba.a)
            );
      },
        ...
        if ($(cal).data('wcolpick').enableAlpha) {
          if (isInternetExplorer()) {
            ...
            $(cal)
              ...
              .alphaBar.attr(
                'style',
                'filter:progid:DXImageTransform.Microsoft.gradient(GradientType=1,startColorstr=0,endColorstr=#' +
                  end +
                  '); -ms-filter:"progid:DXImageTransform.Microsoft.gradient(GradientType=1,startColorstr=0,endColorstr=#' +
                  end +
                  ')";'
              );
          } else {
            var begin = 'rgba(' + rgba.r + ',' + rgba.g + ',' + rgba.b + ',0)',
              end = 'rgba(' + rgba.r + ',' + rgba.g + ',' + rgba.b + ',1)';
          }
          $(cal)
            .data('wcolpick')
            .alphaBar.attr(
              ...
              'background:-webkit-linear-gradient(left,' +
                ...
                ' 100%); background:-moz-linear-gradient(left,' +
                begin +
                ' 0%,' +
                end +
                ' 100%); background:-ms-linear-gradient(left,' +
                begin +
                ' 0%,' +
                end +
                ' 100%); background:-o-linear-gradient(left,' +
                begin +
                ' 0%,' +
                end +
      ...
      change = function () {
        ...
        if (this.parentNode.className.indexOf('_alpha') > 0) {
          ...
          rgba = hexToRgba(
            adaptHex(cal.data('wcolpick').fields.eq(0).val(), cal)
          );
        ...
        cal.data('wcolpick').onChange.apply(cal.parent(), [
          {
            bySetColor: false,
            colorDiv: cal.get(0),
            el: cal.data('wcolpick').el,
            hex: hex.substring(0, 6),
            hexa: hex,
            hsb: cloneHSBA(hsba, false),
            hsba: hsba,
            hsl: cloneHSLA(hsla, false),
            hsla: hsla,
            rgb: cloneRGBA(rgba, false),
            rgba: rgba,
          },
        ]);
      },
          ...
          max:
            this.parentNode.className.indexOf('_hsb_h') > 0
              ...
              : this.parentNode.className.indexOf('_hsb') > 0
                ? 100
                : this.parentNode.className.indexOf('_alpha') > 0
                  ? 100
                  : 255,
          y: ev.pageY,
          field: field,
      ...
      moveIncrement = function (ev) {
        ...
        ev.data.field.val(
          fixVal(Math.round(ev.data.val - ev.pageY + ev.data.y), 0, ev.data.max)
        );
              ...
              Math.round(
                (100 *
                  fixVal(
                    pageX - current.x,
                    0,
                    current.cal.data('wcolpick').size
                  )) /
                  current.cal.data('wcolpick').size
              )
            )
      ...
      moveAlpha = function (ev) {
        ...
        var alpha = Math.round(
          (100 *
            fixVal(pageX - ev.data.x, 0, ev.data.cal.data('wcolpick').size)) /
            ev.data.cal.data('wcolpick').size
        );
        //livePreview = true: update colors | livePreview = false: update only position
        if (ev.data.preview)
          change.apply(
            ev.data.cal.data('wcolpick').fields.eq(7).val(alpha).get(0)
          );
      ...
      moveHue = function (ev) {
        ...
        if (ev.data.preview)
          change.apply(
            ev.data.cal.data('wcolpick').fields.eq(4).val(hue).get(0)
          );
      ...
      moveSelector = function (ev) {
        ...
        if (ev.data.preview)
          ...
          setSelectorPos({ s: saturation, b: brightness }, ev.data.cal.get(0));
        return false;
      },
      ...
      cloneRGBA = function (rgba, withAlpha) {
        ...
        if (withAlpha) {
          if (rgba.a === undefined)
            return { r: rgba.r, g: rgba.g, b: rgba.b, a: 1 };
      ...
      cloneHSBA = function (hsba, withAlpha) {
        ...
        if (withAlpha) {
          if (hsba.a === undefined)
            return { h: hsba.h, s: hsba.s, b: hsba.b, a: 1 };
      ...
      cloneHSLA = function (hsla, withAlpha) {
        ...
        if (withAlpha) {
          if (hsla.a === undefined)
            return { h: hsla.h, s: hsla.s, l: hsla.l, a: 1 };
      ...
      show = function (ev) {
        ...
        try {
          var temp = cal.data('wcolpick').color;
        } catch (e) {
          return this;
        }
        //Polyfill fixes
        ...
        //Fires onBeforeShow
        cal.data('wcolpick').onBeforeShow.apply(this, [
          {
            colorDiv: cal.get(0),
            el: cal.data('wcolpick').el,
          ...
          if (
            !(
              cal
                .data('wcolpick')
                .onShow.apply(this, [
                  { colorDiv: cal.get(0), el: cal.data('wcolpick').el },
                ]) != false
            )
          )
            return this;
        ...
        if (
          overridePos.left === undefined ||
          isNaN(overridePos.left) ||
          overridePos.top === undefined ||
          isNaN(overridePos.top)
        ) {
          //Calculates the correctly position
          var finalPos = { top: 0, left: 0 },
            pos = cal.data('wcolpick').appendedToBody
              ? $(this).offset()
              : $(this).position();
          if (cal.data('wcolpick').position == 'center') {
            //... on center of viewport
            //Positions the color picker on top-left corner of viewport
            finalPos.top =
              pos.top - ($(this).offset().top - $(window).scrollTop());
            finalPos.left =
              pos.left - ($(this).offset().left - $(window).scrollLeft());
            ...
            if (
              outOfViewportHeight(cal, $(this), this) &&
              $(this).offset().top - $(window).scrollTop() >= cal.outerHeight()
            ) {
            ...
            if (outOfViewportWidth(cal, $(this))) {
              if (
                ...
                var outWidth =
                  leftMargin + cal.outerWidth() - $(window).width();
                if (leftMargin > outWidth) {
                  finalPos.left -= outWidth;
                } else {
                  finalPos.left -= leftMargin;
                }
              }
            }
          }
          ...
          cal.css({
            top: overridePos.top + 'px',
            left: overridePos.left + 'px',
          });
        }
        //Fires onShow
        if (
      ...
      outOfViewportHeight = function (cal, wrapEl, domEl) {
        var calViewTop =
          wrapEl.offset().top - $(window).scrollTop() + domEl.offsetHeight; //Top of the color picker in viewport
    ...
    return {
      init: function (opt) {
        ...
        if (typeof opt.color === 'string') opt.color = hexToHsba(opt.color);
          ...
          opt.color = rgbaToHsba(fixRGBA(opt.color));
        else if (
          ...
          opt.color.h !== undefined &&
          opt.color.s !== undefined &&
          opt.color.l !== undefined
        )
          opt.color = hslaToHsba(fixHSLA(opt.color));
        ...
        if (typeof opt.fieldsBackground === 'string')
          opt.fieldsBackground = opt.fieldsBackground.toLowerCase();
        if (typeof opt.submitBackground === 'string')
          opt.submitBackground = opt.submitBackground.toLowerCase();
        ...
        return this.each(function () {
          ...
          if (!$(this).data('wcolpickId')) {
            ...
            if (options.variant == 'small') {
              //Add class according to layout (small)
              cal.addClass(
                'wcolpickS wcolpickS_' +
                  options.layout +
                  (options.enableSubmit
                    ? ''
                    : ' wcolpickS_' + options.layout + '_ns')
              );
              if (!options.enableAlpha)
                cal.addClass(
                  'wcolpickS_noalpha wcolpickS_' +
                    options.layout +
                    '_noalpha' +
                    (options.enableSubmit
                      ? ''
                      : ' wcolpickS_' + options.layout + '_noalpha_ns')
                ); //Disable alpha channel, if requested
            } else if (options.variant == 'extra-large') {
              //Add class according to layout (extra-large)
              cal.addClass(
                'wcolpickXL wcolpickXL_' +
                  options.layout +
                  (options.enableSubmit
                    ? ''
                    : ' wcolpickXL_' + options.layout + '_ns')
              );
              if (!options.enableAlpha)
                cal.addClass(
                  'wcolpickXL_noalpha wcolpickXL_' +
                    options.layout +
                    '_noalpha' +
                    (options.enableSubmit
                      ? ''
                      : ' wcolpickXL_' + options.layout + '_noalpha_ns')
                ); //Disable alpha channel, if requested
            } else {
              //Add class according to layout (default -> standard)
              cal.addClass(
                'wcolpick_' +
                  options.layout +
                  (options.enableSubmit
                    ? ''
                    : ' wcolpick_' + options.layout + '_ns')
              );
              if (!options.enableAlpha)
                cal.addClass(
                  'wcolpick_noalpha wcolpick_' +
                    options.layout +
                    '_noalpha' +
                    (options.enableSubmit
                      ? ''
                      : ' wcolpick_' + options.layout + '_noalpha_ns')
                ); //Disable alpha channel, if requested
            }
            ...
            if (options.compactLayout) {
              if (options.variant == 'small') {
                //Add class according to layout (small)
                cal.addClass(
                  'wcolpickS_compact wcolpickS_compact_' +
                    options.layout +
                    (options.enableSubmit
                      ? ''
                      : ' wcolpickS_compact_' + options.layout + '_ns')
                );
                if (!options.enableAlpha)
                  cal.addClass(
                    'wcolpickS_compact_noalpha wcolpickS_compact_' +
                      options.layout +
                      '_noalpha' +
                      (options.enableSubmit
                        ? ''
                ...
                //Add class according to layout (extra-large)
                cal.addClass(
                  'wcolpickXL_compact wcolpickXL_compact_' +
                    options.layout +
                    (options.enableSubmit
                      ? ''
                      : ' wcolpickXL_compact_' + options.layout + '_ns')
                );
                if (!options.enableAlpha)
                  cal.addClass(
                    'wcolpickXL_compact_noalpha wcolpickXL_compact_' +
                      options.layout +
                      '_noalpha' +
                      (options.enableSubmit
                        ? ''
                ...
                //Add class according to layout (default -> standard)
                cal.addClass(
                  'wcolpick_compact wcolpick_compact_' +
                    options.layout +
                    (options.enableSubmit
                      ? ''
                      : ' wcolpick_compact_' + options.layout + '_ns')
                );
                if (!options.enableAlpha)
                  cal.addClass(
                    'wcolpick_compact_noalpha wcolpick_compact_' +
                      options.layout +
                      '_noalpha' +
                      (options.enableSubmit
                        ? ''
                        : ' wcolpick_compact_' + options.layout + '_noalpha_ns')
                  ); //Disable alpha channel, if requested
              }
            }
            //Loading the choosen color scheme
            if (options.colorScheme.indexOf('light') == 0) {
              //All light color schemes start with "light"
              cal.addClass('wcolpick_light'); //Loading default light color scheme
              if (options.colorScheme != 'light')
                cal.addClass('wcolpick_' + options.colorScheme); //Loading light-based color scheme
              //INFO: You can implement light-based color schemes, in css, naming them: light-[name] (IMPORTANT: Use only lowercase names!)
            } else if (options.colorScheme.indexOf('dark') == 0) {
              ...
              if (options.colorScheme != 'dark')
                cal.addClass('wcolpick_' + options.colorScheme); //Loading dark-based color scheme
              //INFO: You can implement dark-based color schemes, in css, naming them: dark-[name] (IMPORTANT: Use only lowercase names!)
            } else {
              //If the scheme does not starts with "light" or "dark"
            ...
            if (options.arrowsColor == 'light')
              cal.addClass('wcolpick_lightArrs');
            else if (options.arrowsColor == 'dark')
              cal.addClass('wcolpick_darkArrs');
            //Change color scheme for checkerboards, if requested
            if (options.checkersColor == 'light')
              cal.addClass('wcolpick_lightCheckerboards');
            else if (options.checkersColor == 'dark')
              cal.addClass('wcolpick_darkCheckerboards');
            //Change color scheme for submit button, if requested
            if (options.submitColor == 'light')
              cal.addClass('wcolpick_lightSubmit');
            else if (options.submitColor == 'dark')
              cal.addClass('wcolpick_darkSubmit');
            ...
            for (var i = 0; i < 8; i++)
              options.fields.eq(i).prop('readonly', options.readonlyFields);
            if (options.readonlyHexField)
              options.fields.eq(0).prop('readonly', options.readonlyHexField);
            ...
            if (options.fieldsBackground != 'default') {
              ...
              for (var i = 0; i < 8; i++)
                options.fields.eq(i).parent().css('background', colstr);
            }
            if (options.submitBackground != 'default')
              options.submit.css(
                'background',
                encodeToCSS(options.submitBackground)
              );
            if (options.backgroundColor != 'default')
              cal.css('background', encodeToCSS(options.backgroundColor));
            if (options.borderColor != 'default')
              cal.css('borderColor', encodeToCSS(options.borderColor));
            //Setup restoreOriginal to current color's click event
            cal
              .find('div.wcolpick_field_arrs')
              .mousedown(downIncrement)
              .end()
              .find('div.wcolpick_current_color')
              .click(restoreOriginal);
            //Setup color selector
            options.selector = cal
              .find('div.wcolpick_color')
              .on('mousedown touchstart', downSelector);
            options.selectorIndic = options.selector.find(
              'div.wcolpick_selector_outer'
            );
            ...
            cal
              .find('div.wcolpick_alpha')
              .on('mousedown touchstart', downAlpha);
      ...
      setColpickColor: function (col, setCurrent) {
        if (col !== undefined) {
          //If color is undefined, do nothing!
          ...
          return this.each(function () {
            if ($(this).data('wcolpickId')) {
              ...
              if (setCurrent) {
                if (
                  compareHSBA(col, cal.data('wcolpick').color) &&
                  compareHSBA(col, cal.data('wcolpick').origColor)
                )
                  return this;
              } else {
                if (compareHSBA(col, cal.data('wcolpick').color)) return this;
              }
              //Setup new color
      ...
      getColpickColor: function (type, getCurrent) {
        ...
        //Getting the color
        var col = getCurrent
          ? cloneHSBA(cal.data('wcolpick').origColor, true)
          : cloneHSBA(cal.data('wcolpick').color, true);
        if (type.indexOf('rgb') != -1) {
          ...
          var hsla = hsbaToHsla(col);
          return withAlpha
            ? { h: hsla.h, s: hsla.s, l: hsla.l, a: hsla.a }
            : { h: hsla.h, s: hsla.s, l: hsla.l };
        } else if (type.indexOf('hex') != -1)
          return withAlpha ? hsbaToHex(col) : hsbaToHex(col).substring(0, 6);
        else
          return withAlpha
            ? { h: col.h, s: col.s, b: col.b, a: col.a }
  ...
  var hexToRgba = function (hex) {
    ...
    var rgba = {
      r: hexI >>> 24,
      g: (hexI & 0x00ff0000) >>> 16,
      b: (hexI & 0x0000ff00) >>> 8,
      a: (hexI & 0x000000ff) / 255,
    };
    return rgba;
  };
  ...
  var hsbaToHsla = function (hsba) {
    ...
    var saturation =
      lightness != 0 && lightness != 1
        ? (hsba.b * hsba.s) / (1 - Math.abs(2 * lightness - 1))
        : 0;
    return fixHSLA({
      h: hsbaL.h,
      s: saturation * 100,
  ...
  var hslaToHsba = function (hsla) {
    ...
    var hslaL = { h: hsla.h, s: hsla.s / 100, l: hsla.l / 100, a: hsla.a };
    var brightness =
      (2 * hslaL.l + hslaL.s * (1 - Math.abs(2 * hslaL.l - 1))) / 2;
    var saturation =
      brightness != 0 ? (2 * (brightness - hslaL.l)) / brightness : 0;
    return fixHSBA({
      h: hslaL.h,
      s: saturation * 100,
      b: brightness * 100,
  ...
  var decodeFromCSS = function (colstr) {
    ...
    if (colstr.indexOf('rgb') != -1) {
      var elems = colstr
        .substring(colstr.indexOf('(') + 1, colstr.indexOf(')'))
        .split(',');
      return fixRGBA({
        r: parseInt(elems[0]),
        g: parseInt(elems[1]),
        b: parseInt(elems[2]),
        a: parseFloat(elems[3]),
      });
      ...
      var elems = colstr
        ...
        .split(',');
      return fixHSLA({
        h: parseFloat(elems[0]),
        s: parseFloat(elems[1]),
        l: parseFloat(elems[2]),
        a: parseFloat(elems[3]),
      });
    } else {
