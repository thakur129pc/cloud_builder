...
$(document).ready(function () {
  $.getJSON('data/database-details.json', function (initData) {
    ...
    $('[name="database"]').on('change', function () {
      ...
      $('.' + value + ' a').tab('show');
      // $.each(mainData, function (key, item) {
      // 	if (key == value) {
      // 		tableLists = item['tables'];
      // 		sub_schemas = item['sub_schemas'] != undefined ? item['sub_schemas'] : [];
      // 		currentData = item;
      // 	}
      // });
      ...
      // 		const { name, description, tag, group, db_permission, number_of_records, access_level, sub_schema } = item;
      // 		let initTag = tag;
      // 		let tagStatus = '';
      // 		let initDesc = description;
      // 		let descStatus = '';

      // 		let html = `<tr data-id="${name}" data-desc="${initDesc}" data-tag="${initTag}" data-ai-access="${access_level}" class="table-list-inner"> <td width="7%"> <div class="c-checkbox"> <label> <input type="checkbox"  name="sub_checkbox[]" class="checkall prompt_check"> <span class="fa fa-check"></span> </label> </div> </td> <td width="15%"> ${name} </td> <td width="34%" class="editText ${descStatus}" data-name="description"> ${initDesc} </td> <td width="15%"> ${sub_schema.join(', ')} </td> <td width="14%"> ${access_level ? "Yes" : "No"}  </td> <td width="15%" class="editText ${tagStatus}" data-name="tag"> ${initTag} </td> </tr>`;
