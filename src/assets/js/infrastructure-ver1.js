...
$(document).ready(function () {
  ...
  $.getJSON('data/private-ai-data.json', function (privateAIData) {
    ...
    var privateAIList = $('#cost-explorer').DataTable({
      iDisplayLength: 25, // Set the default number of rows to display
    });
    tableDetails('cost-explorer');
    ...
    // 	let title = $('#privateAIList thead th').eq($(this).index()).text().trim();
    ...
    // 	let title = $('#summary-table-list thead th').eq($(this).index()).text().trim();
    // 	if (title != '' && title != 'Action' && title != 'Use for chat') {
    // 		$(this).html('<input type="text" class="filter" placeholder="' + title + '" />');
    // 	}
    // });

    // var summaryTable = $('#summary-table-list').DataTable({
    // 	"iDisplayLength": 25, // Set the default number of rows to display
