...
$(document).ready(function () {
  ...
  $.getJSON('data/database-details.json', function (db) {
    ...
    if (url.indexOf('e=') != -1) {
      ...
      let editId = params.get('e');
      let viewData = mainData[editId];
      ...
      if (viewData.multiDatabaseArr != undefined && viewData.multiDatabaseArr) {
        ...
        $('[name="multiDatabaseArr"]').trigger('change');
      }
        ...
        $.each(queryData, function (key, curItem) {
          ...
          $.each(queryNames, function (key, item) {
            ...
            if (!notReqQuery.includes(item.name)) {
              ...
              queryOption += `<option value="${item.name}" ${sel} data-id="${item.id}" data-name="${item.name}" data-dbId="${item.dbId}">${item.name}</option>`;
              status.push(true);
            } else {
              status.push(false);
            }
