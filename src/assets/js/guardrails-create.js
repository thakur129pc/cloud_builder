$(document).ready(function () {
  ...
  $.getJSON('data/guardrails.json', function (db) {
    console.log('db: ', db);
    ...
    //     $('#input-list').DataTable().row.add($(`<tr data-id="${id}" class="input-guardrails"><td class="guard-input-id"><a href="javascript:void(0)">${id}</a></td><td class="desc">${description}</td><td class="token">${token}</td><td class="onfail">${onfail}</td></tr>`)).draw();
    // });
    ...
    //     let { id, description, token, onfail } = item;
    //     $('#output-list').DataTable().row.add($(`<tr data-id="${id}" class="input-guardrails"><td class="guard-output-id"><a href="javascript:void(0)">${id}</a></td><td class="desc">${description}</td><td class="token">${token}</td><td class="onfail">${onfail}</td></tr>`)).draw();
    // });
    ...
    //     let { id, description, token, onfail } = item;
    //     $('#filter-list').DataTable().row.add($(`<tr data-id="${id}" class="input-guardrails"><td class="guard-filter-id"><a href="javascript:void(0)">${id}</a></td><td class="desc">${description}</td><td class="token">${token}</td><td class="onfail">${onfail}</td></tr>`)).draw();
    // });
    ...
    //     });
    //     inputValue = document.querySelector('#tag_value').value;

    //     if (inputValue != '' && inputValue != null && !array_tag.includes(inputValue)) {
    //         $('#tag_value').css('color', 'black');
    //         tags.push(inputValue);

    //         addTags();
    //         document.querySelector('#tag_value').value = '';
    //         jQuery('div#popup_holder').addClass('hidden');
    //     } else {
    //         $('#tag_value').css('color', 'red');
    //     }
    ...
    $('#save-confirm-btn').on('click', function () {
      ...
      if (FormInstance.isValid()) {
        $('[name="tokens"]').val(
          inputTableData.length +
            outputTableData.length +
