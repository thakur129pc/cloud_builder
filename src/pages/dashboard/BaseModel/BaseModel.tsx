                          ...
                          <tbody>
                            {modelsData.map((model, index) => (
                              <tr
                                key={index}
                                role="row"
                                data-id={model.llm_family_id}
                                className={index % 2 === 0 ? 'even' : 'odd'}
                              >
                                <td width="20%" className="sorting_1">
                                  <Link
                                    onClick={() => {
                                      dispatch(updateLLMList(model));
                                    }}
                                    // to={`/basemodelcreate/${model.llm_family_id}`}
                                    to={`/basemodelcreate/${model.id}`}
                                  >
                                    {model.llm_name}
                                  </Link>
                                </td>
                                <td>
                                  {model.llm_type || 'No description available'}
                                </td>
                              </tr>
                            ))}
                          </tbody>
