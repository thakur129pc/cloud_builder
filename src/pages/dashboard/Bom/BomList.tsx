...
import { BomDetails } from '../../../types/ItemsTypes';

const BomListComp: React.FC = () => {
  ...
  const [bomList, setBomList] = useState<BomDetails[]>([]);

  useEffect(() => {
    const fetclAllBom = async () => {
      try {
        dispatch(updateLoadingState(true));
        const response = await axios.get(API.API_CB + 'admin/bom', {
          headers: {
                            ...
                            {bomList?.map((bom, index) => (
                              ...
                              >
                                <td width="20%" className="sorting_1">
                                  <Link
                                    onClick={() => {
                                      dispatch(updateSelectedTestMachine(bom));
                                    }}
                                    to={`/bomcreate/${bom?.bom_id}`}
                                  >
                                    {bom.bom_id}
                                  </Link>
                                </td>
